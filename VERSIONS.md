...
## v1.9.6 (08-26-2026)
- Fixed CARTO key propagation on the authenticated `/map` route used after Cloudflare Turnstile verification. The public route and container environment could both have a valid key while `/map` left the template placeholder unresolved, causing the frontend to hide the dark-mode button.
- Fixed map-link embeds so generated preview images use the current deployment hostname and therefore the same `CARTO_BASEMAP_KEY`, rather than a stale `SITE_URL` deployment that could still render CARTO's API-key-required watermark.
- Fixed issue #96 after CARTO began requiring API keys for Dark Matter tiles on August 26, 2026. Added `CARTO_BASEMAP_KEY` support for dark mode, satellite labels, and dark social preview images; CARTO's free key covers up to 5 million tile requests per calendar month. The documented browser-direct integration keeps the key out of Git and logs but leaves it visible in tile requests, so it must remain scoped to this project and not be reused elsewhere. Until a suitable replacement is found, deployments without a key hide the dark-mode control, fall previews back to OpenStreetMap, and keep satellite imagery available without the CARTO label overlay. See [Get and configure a CARTO API key for dark mode](howto.md#get-and-configure-a-carto-api-key-for-dark-mode).
